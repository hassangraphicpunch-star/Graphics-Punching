...
export const ServicesSection: React.FC<ServicesSectionProps> = ({ onSelectServiceForQuote }) => {
  ...
  const categories = [
    ...
    { id: 'vector', label: 'Vector Artwork', count: allPackages.filter(p => p.category === 'vector').length },
    { id: 'digitizing', label: 'Logo Digitizing', count: allPackages.filter(p => p.category === 'digitizing').length },
  ];
