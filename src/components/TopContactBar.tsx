...
import { Phone, Mail, Facebook, Instagram, Globe, Sparkles, Youtube, Linkedin } from 'lucide-react';
...
export const TopContactBar: React.FC = () => {
  ...
  if (!settings.sections.topContactBar) {
    return null;
  }
      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4">
        ...
        <div className="flex items-center flex-wrap justify-center sm:justify-start gap-2.5 sm:gap-5 w-full sm:w-auto">
          ...
          >
            ...
            <span className="tracking-wide whitespace-nowrap">{settings.contact.phone}</span>
          </a>
          ...
          >
            ...
            <span className="tracking-wide truncate">{settings.contact.email}</span>
          </a>
        </div>

        {/* Center / Right: Announcement Tag or Social Links */}
        <div className="flex items-center gap-3 shrink-0">
          {settings.branding.announcementBarEnabled && settings.branding.announcementBarText && (
            <a
              href={settings.branding.announcementBarLink || '#/contact'}
              className="hidden md:inline-flex items-center gap-1 text-[11px] font-bold text-[#FFC400] bg-[#FFC400]/10 border border-[#FFC400]/20 px-2.5 py-0.5 rounded-full hover:bg-[#FFC400]/20 transition-colors"
            >
              <span>{settings.branding.announcementBarText}</span>
            </a>
          )}
