 ...
 * canonical links, OpenGraph, and Twitter tags on route changes.
 */
export const SEOHead: React.FC<SEOHeadProps> = ({
  ...
  useEffect(() => {
    ...
    const finalKeywords = keywords || config.keywords || 'embroidery digitizing, vector artwork, screen printing';
    const finalPath = canonicalPath || config.canonicalPath || (page === 'home' ? '/' : `/#/${page}`);

    // Build absolute URL for canonical & OG
    ...
    // 4. OpenGraph Tags (Facebook, LinkedIn, iMessage, etc.)
    setMetaTag('property', 'og:site_name', 'Graphics Punching');
  ...
  }, [page, title, description, keywords, canonicalPath, image]);
